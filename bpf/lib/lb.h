...
} LB6_REVERSE_NAT_MAP_V2 __section_maps_btf;
...
struct {
	...
	__uint(max_entries, CILIUM_LB_SERVICE_MAP_MAX_ENTRIES);
	__uint(map_flags, CONDITIONAL_PREALLOC);
} LB4_SERVICES_MAP_V3 __section_maps_btf;
...
lb6_lookup_rev_nat_entry(struct __ctx_buff *ctx __maybe_unused, __u16 index)
{
	...
	return map_lookup_elem(&LB6_REVERSE_NAT_MAP_V2, &index);
}
...
lb4_lookup_rev_nat_entry(struct __ctx_buff *ctx __maybe_unused, __u16 index)
{
	...
	__u32 rev_nat_id = index;

	return map_lookup_elem(&LB4_REVERSE_NAT_MAP_V2, &rev_nat_id);
}
				       ...
				       const bool scope_switch)
{
	...
	key->scope = LB_LOOKUP_SCOPE_EXT;
	key->backend_slot = 0;
	svc = map_lookup_elem(&LB4_SERVICES_MAP_V3, key);
...
{
	return map_lookup_elem(&LB4_SERVICES_MAP_V3, key);
}
